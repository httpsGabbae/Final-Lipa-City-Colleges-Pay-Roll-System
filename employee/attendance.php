...
<?php if (!$todayRecord || !$todayRecord['time_in']): ?><form method="post"><?php echo csrf_field(); ?><input type="hidden" name="action" value="time_in"><button class="attendance-button time-in" type="submit"><span class="attendance-button-icon">→</span><span><b>Time In</b><small>Start your workday</small></span></button></form>
<?php elseif (!$todayRecord['time_out']): ?><div class="attendance-status done"><span>✓</span><div><b>Timed in at <?php echo e(date('h:i A', strtotime($todayRecord['time_in']))); ?></b><small>You are currently clocked in.</small></div></div><form method="post"><?php echo csrf_field(); ?><input type="hidden" name="action" value="time_out"><button class="attendance-button time-out" type="submit"><span class="attendance-button-icon">←</span><span><b>Time Out</b><small>End your workday</small></span></button></form>
